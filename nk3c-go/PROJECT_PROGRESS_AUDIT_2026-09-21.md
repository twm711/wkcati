...
本轮新增 `sys_org`、`sys_group`，并为用户/项目增加 `org_id`、`group_id`；项目列表、项目访问和派样开始按坐席组过滤。当前仍属于基础模型，尚无组织/分组管理 API、技能组/队列调度和完整历史数据迁移。

仍未完成：历史 IVR 日志归属校验、全链路跨租户自动化测试、组织/分组管理和技能组数据域。当前仍不能上线多租户生产。
