...
本轮开始建立拨号策略模型：新增 `cti_dial_strategy` 及 `/api/monitor/dial-strategies` 查询/配置接口，支持 `PREDICTIVE`、`PROGRESSIVE`、`PREVIEW`、最大并发、呼损目标和预览秒数；当前已接入项目级最大并发限制：派样和等待调度均会统计项目 LEASED 任务并在达到 max_concurrent 时进入 WAITING；新增 PREVIEW 预览接口：锁定样本 30 秒、支持确认转 LEASED 话务或跳过回池；确认接口直接返回完整问卷上下文，React 坐席工作台确认后直接进入问卷作答状态；新增预览样本、倒计时、确认拨打和跳过操作；新增 `/api/monitor/dial-runtime` 返回项目拨号模式、activeCalls、maxConcurrent、availableAgents、headroom 和 running 状态；新增自动拨号任务领取器：每 2 秒在 SIP 外呼腿已挂载时，按 PROGRESSIVE/PREDICTIVE 策略、项目 max_concurrent、READY 坐席、队列 capacity 和可用样本原子创建 LEASED 话务，并异步调用媒体 Dial；PREDICTIVE 根据历史接通率、READY 坐席数和 abandon_target 计算动态并发上限；PREDICTIVE 当前已接入 15 分钟可配置滑动窗口、最小样本量保护、EWMA 风格倍率平滑、平均通话时长/接通等待时长反馈、基础动态并发上限和 BREAKOFF 呼损率反馈；自动拨号在已配置数据库线路时会检查线路容量、OPEN 熔断状态和最近 15 分钟线路失败率；最近窗口至少 5 通话且 BUSY/NA/REFUSE/BREAKOFF 占比达到 50% 的线路会被自动跳过；失败率达到 20% 且未达到跳过阈值的线路会被降权，健康线路优先承担新任务；线路新增 `rate_limit_per_minute`（默认 30），领取任务时按最近 1 分钟已创建呼叫数限制单线路速率；并根据线路连续失败状态动态调整有效速率：`failure_streak>=2` 降至 50%；恢复成功后进入 5 分钟冷却斜坡，先以 75% 速率运行，冷却结束且无新失败后再提升至 125%，仍受配置限流值约束；新增 `/api/monitor/line-runtime`，按可配置时间窗口返回线路总呼叫、接通、BREAKOFF 呼损、呼损率和最近话务时间，并返回配置速率、当前有效速率、failure streak、熔断状态和恢复冷却剩余秒数；React + Ant Design 监控墙已展示线路呼损、当前/配置速率、失败连击、熔断状态和恢复冷却；机构管理员可在监控墙修改线路基础分钟速率，前端要求填写调整原因并二次确认；后端通过独立接口执行 0～10000 边界校验，并将操作者、旧值、新值、原因、时间写入 `cti_line_rate_audit`；新增 `/api/monitor/lines/rate-audits` 查询接口，监控墙可按线路打开速率调整历史抽屉；动态降速和恢复斜坡仍由系统保留；呼损高于 abandon_target 时收缩倍率，明显低于目标时小幅增加；长通话或长接通等待会进一步收缩倍率；倍率具有可配置的最小/最大边界并持久化 current_multiplier；运行态接口同时返回 abandonRate；但仍未形成生产级呼损闭环，渐进节奏、预测倍率校准和运营商真实验证仍需继续。
...
- 新增 `cti_waiting_task` 等待队列表；坐席队列容量已满时不再直接丢弃请求，而是幂等进入 WAITING；容量释放后后台每 30 秒自动为等待任务选择样本、创建话务和 LEASED 任务，并标记 ASSIGNED；结果提交后仅在存在 WAITING 项时同步触发一次调度；等待调度使用独立 worker lock，避免与租约回收互相阻塞；每轮每个坐席最多自动分配一个等待任务，降低单坐席长期霸占调度批次的风险；新增等待任务查询和取消接口及 React 等待队列页面；新增等待调度分配 E2E 测试；`go test ./...`、`npx tsc --noEmit` 全部通过。
...
PREDICTIVE 多轮测试增加第四轮短样本保护：将窗口历史减少到 5 条后再次调用生产 Claim，断言倍率保持第三轮约 0.862、不会重置或剧烈调整，且 last_sample_count 保留上一轮 20；真实 MySQL DSN 未配置，仍待外部执行。
...
已在沙箱安装 MariaDB 11.8.6（MySQL 协议兼容），创建本地 `nk3c_test` 数据库和专用测试账号，并配置 `NK3C_MYSQL_DSN` 执行真实数据库测试。首次执行发现迁移器将整份 MySQL 迁移文件作为多语句发送，MariaDB 在 001 失败；随后修复迁移器为逐条执行分号分隔语句，并先移除 SQL 行注释，避免 008 注释吞掉下一条 ALTER。真实 MySQL 测试随后全部通过：新鲜库迁移、线路速率并发、租约回收/领取竞争、样本领取并发、完整 claim 事务、HALF_OPEN 探针并发及迁移顺序测试均 PASS。

同时修复 MySQL predictive 集成测试的两个测试隔离/断言问题：只在实际 claimed 时计数，清理固定测试 ID 的队列/样本，并将测试窗口扩展至 30 分钟避免边界时间记录滑出 15 分钟窗口。配置 DSN 后再次执行 `go test ./...`，全部 Go 包通过。真实验证仍仅覆盖本地单实例 MariaDB；生产 MySQL 多实例、外部 SIP 和长稳测试仍未完成。
