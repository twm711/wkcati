...
func (s *Service) ReserveOutboundLine(callID int64) (OutboundLine, error) {
	...
	for i := 0; i < 8; i++ {
		...
		var port, rateLimit int
		now := store.NowFor(s.db.Driver)
		cutoff := store.TimeFor(s.db.Driver, time.Now().UTC().Add(-15*time.Minute))
		recoveryCutoff := store.TimeFor(s.db.Driver, time.Now().UTC().Add(-5*time.Minute))
		err := s.db.QueryRow(`SELECT id,line_no,COALESCE(host,''),port,circuit_state,rate_limit_per_minute FROM cti_outbound_line l WHERE tenant_id=? AND enabled=1 AND active_calls<capacity AND (rate_limit_per_minute<=0 OR (SELECT COUNT(*) FROM cti_call_record r JOIN prj_project rp ON rp.id=r.project_id WHERE rp.tenant_id=l.tenant_id AND r.caller_no=l.line_no AND r.begin_time>=?) < rate_limit_per_minute * CASE WHEN failure_streak>=2 THEN 0.5 WHEN last_recovery_at IS NOT NULL AND last_recovery_at>=? THEN 0.75 WHEN failure_streak=0 THEN 1.25 ELSE 1.0 END) AND (circuit_state='CLOSED' OR (circuit_state='OPEN' AND opened_until IS NOT NULL AND opened_until<=?)) AND (circuit_state='OPEN' OR NOT EXISTS (SELECT 1 FROM cti_call_record c JOIN prj_project p ON p.id=c.project_id WHERE p.tenant_id=l.tenant_id AND c.caller_no=l.line_no AND c.status='CLOSED' AND c.end_time>=? GROUP BY c.caller_no HAVING COUNT(*)>=5 AND SUM(CASE WHEN c.result_code IN ('BREAKOFF','BUSY','NA','REFUSE') THEN 1 ELSE 0 END)*2>=COUNT(*))) ORDER BY CASE WHEN (SELECT COUNT(*) FROM cti_call_record c2 JOIN prj_project p2 ON p2.id=c2.project_id WHERE p2.tenant_id=l.tenant_id AND c2.caller_no=l.line_no AND c2.status='CLOSED' AND c2.end_time>=?) >= 5 AND (SELECT COALESCE(SUM(CASE WHEN c3.result_code IN ('BREAKOFF','BUSY','NA','REFUSE') THEN 1 ELSE 0 END),0) FROM cti_call_record c3 JOIN prj_project p3 ON p3.id=c3.project_id WHERE p3.tenant_id=l.tenant_id AND c3.caller_no=l.line_no AND c3.status='CLOSED' AND c3.end_time>=?)*5 >= (SELECT COUNT(*) FROM cti_call_record c4 JOIN prj_project p4 ON p4.id=c4.project_id WHERE p4.tenant_id=l.tenant_id AND c4.caller_no=l.line_no AND c4.status='CLOSED' AND c4.end_time>=?) THEN 1 ELSE 0 END, priority,id LIMIT 1`, tenant, cutoff, recoveryCutoff, now, cutoff, cutoff, cutoff, cutoff).Scan(&id, &no, &host, &port, &state, &rateLimit)
		if err != nil {
			return line, err
		}
		if state == "OPEN" {
...
func (s *Service) resultCore(agentID, callID int64, resultCode string) (map[string]interface{}, string, error) {
	...
	err := s.db.Tx(func(tx *sql.Tx) error {
		...
		if category == "FAIL" {
			...
			_, _ = tx.Exec(`UPDATE cti_outbound_line SET failure_streak=0,circuit_state='CLOSED',opened_until=NULL,last_recovery_at=? WHERE tenant_id=? AND line_no=?`, ts, tenantID, callerNo)
		}
		if lineID > 0 {
			var newState string
