...
func Build(db *store.DB) *App {
	...
	api := e.Group("/api", a.RequireAuth())
	{
		...
		api.GET("/agent/preview", ag.PreviewTask)
		api.POST("/agent/preview/:taskId/confirm", ag.ConfirmPreview)
		api.POST("/agent/preview/:taskId/skip", ag.SkipPreview)
		...
		api.GET("/monitor/lines/rate-audits", mo.LineRateAudits)
