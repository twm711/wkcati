...
func TestRecoverStaleTasksAfterRestart(t *testing.T) {
	ts, db, _ := setup(t)
	defer ts.Close()
	...
	if _, err := db.Exec(`INSERT INTO cti_sample_task(id,project_id,sample_id,call_id,assigned_user_id,status,leased_at,lease_until)
		VALUES(9002,1,101,1,2,'LEASED','2026-09-21T00:00:00+00:00','2999-01-01T00:00:00+00:00')`); err != nil {
		t.Fatal(err)
	}
	n, err := agent.New(db).RecoverStaleTasks()
	if err != nil || n != 1 {
		t.Fatalf("陈旧任务恢复数量错误 n=%d err=%v", n, err)
	}
	var taskStatus, sampleStatus, callStatus, result string
	_ = db.QueryRow(`SELECT status FROM cti_sample_task WHERE id=9002`).Scan(&taskStatus)
	_ = db.QueryRow(`SELECT status FROM smp_sample WHERE id=101`).Scan(&sampleStatus)
	_ = db.QueryRow(`SELECT status,COALESCE(result_code,'') FROM cti_call_record WHERE id=1`).Scan(&callStatus, &result)
	if taskStatus != "EXPIRED" || sampleStatus != "IDLE" || callStatus != "CLOSED" || result != "NA" {
		t.Fatalf("陈旧任务恢复状态错误 task=%s sample=%s call=%s result=%s", taskStatus, sampleStatus, callStatus, result)
	}
}
