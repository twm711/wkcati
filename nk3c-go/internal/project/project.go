...
func (s *Service) List(c *gin.Context) {
	rows, err := s.db.Query(`SELECT p.id,p.project_code,p.project_name,p.status,p.questionnaire_id,
		q.version,q.status FROM prj_project p LEFT JOIN qnr_questionnaire q ON q.id=p.questionnaire_id ORDER BY p.id`)
	if err != nil {
...
type createReq struct{ Name string `json:"name" binding:"required"` }
...
func (s *Service) Create(c *gin.Context) {
	...
	if _, err := s.db.Exec(`INSERT INTO prj_project VALUES(?,?,?,?,?)`, pid, code, req.Name, "DRAFT", nid); err != nil {
		rinfo.GinFail(c, rinfo.CodeInternal, err.Error()); return
	}
...
func (s *Service) Detail(c *gin.Context) {
	...
	if err := s.db.QueryRow(`SELECT id,project_code,project_name,status,questionnaire_id FROM prj_project WHERE id=?`, pid).
		...
		rinfo.GinFail(c, rinfo.CodeNotFound, "项目不存在"); return
	}
	...
	for srows != nil && srows.Next() {
		...
		_ = srows.Scan(&st, &n); sm[st] = n
	}
	if srows != nil { srows.Close() }
	vers := row{}
	...
	for vrows != nil && vrows.Next() {
		...
		_ = vrows.Scan(&v, &n); vers[v] = n
	}
	if vrows != nil { vrows.Close() }
...
func (s *Service) AddQuestion(c *gin.Context) {
	...
	if req.QType == "" { req.QType = "single" }
	if req.QType != "single" && req.QType != "number" && req.QType != "text" {
		rinfo.GinFail(c, rinfo.CodeParam, "qType 须为 single/number/text"); return
	}
	var status string
	var qid int64
	...
	if status != "DRAFT" {
		rinfo.GinFail(c, rinfo.CodeState, "项目 "+status+"，问卷不可编辑（发布后版本冻结）"); return
	}
	if req.QType == "single" && len(req.Options) == 0 {
		rinfo.GinFail(c, rinfo.CodeParam, "single 题须提供 options"); return
	}
	var newQID int64
	...
	var mn, mx interface{}
	if req.QType == "number" {
		if req.Min != nil { mn = *req.Min }
		if req.Max != nil { mx = *req.Max }
	}
...
func (s *Service) SetQuota(c *gin.Context) {
	...
	if status != "DRAFT" {
		rinfo.GinFail(c, rinfo.CodeState, "项目已是 "+status+"，配额不可编辑"); return
	}
	for _, cell := range req.Cells {
		var qt string
		err := s.db.QueryRow(`SELECT q_type FROM qnr_question WHERE id=? AND qnr_id=?`, cell.QuestionID, qid).Scan(&qt)
		if err != nil {
			rinfo.GinFail(c, rinfo.CodeParam, fmt.Sprintf("题目 %d 不属于本项目问卷", cell.QuestionID)); return
		}
		if qt != "single" || len(cell.InList) == 0 {
			rinfo.GinFail(c, rinfo.CodeParam, "配额格仅支持 single 题"); return
		}
		for _, oid := range cell.InList {
			...
			if one != 1 {
				rinfo.GinFail(c, rinfo.CodeParam, fmt.Sprintf("选项 %d 不属于题目 %d", oid, cell.QuestionID)); return
			}
		}
		if cell.Target < 1 {
			rinfo.GinFail(c, rinfo.CodeParam, "target 须 ≥1"); return
		}
	}
	total := int64(0)
	err := s.db.Tx(func(tx *sql.Tx) error {
		...
		if err == nil {
			if _, err := tx.Exec(`DELETE FROM qnr_quota_cell WHERE quota_id=?`, old); err != nil { return err }
			if _, err := tx.Exec(`DELETE FROM qnr_quota WHERE id=?`, old); err != nil { return err }
		}
		var qnID int64
		...
		for _, cell := range req.Cells {
			...
			if _, err := tx.Exec(`INSERT INTO qnr_quota_cell VALUES(?,?,?,?,0)`, cid, qnID, string(cond), cell.Target); err != nil { return err }
		}
		if _, err := tx.Exec(`INSERT INTO qnr_quota VALUES(?,?,?,?)`, qnID, qid, req.Name, total); err != nil { return err }
		return nil
	})
	if err != nil {
		rinfo.GinFail(c, rinfo.CodeInternal, err.Error()); return
...
func (s *Service) Publish(c *gin.Context) {
	...
	if status != "DRAFT" {
		rinfo.GinFail(c, rinfo.CodeState, "项目已是 "+status+"，无需重复发布"); return
	}
	var n int
	_ = s.db.QueryRow(`SELECT COUNT(*) FROM qnr_question WHERE qnr_id=?`, qid).Scan(&n)
	if n == 0 {
		rinfo.GinFail(c, rinfo.CodeParam, "问卷至少需要 1 道题"); return
	}
...
func (s *Service) Revise(c *gin.Context) {
	...
	if status != "RUNNING" {
		rinfo.GinFail(c, rinfo.CodeState, "仅 RUNNING 项目可修订（当前 " + status + "；DRAFT 请用加题端点）"); return
	}
	...
	for _, spec := range req.AddQuestions {
		if spec.QType == "" { spec.QType = "single" }
		...
		if spec.QType == "number" {
			if spec.Min != nil { mn = *spec.Min }
			if spec.Max != nil { mx = *spec.Max }
		}
		if _, err := s.db.Exec(`INSERT INTO qnr_question VALUES(?,?,?,?,?,?,?,?)`,
...
type statusReq struct{ Action string `json:"action" binding:"required"` }
...
func (s *Service) Status(c *gin.Context) {
	...
	if !ok {
		rinfo.GinFail(c, rinfo.CodeParam, "action 须为 pause/resume/finish"); return
	}
	var status string
	...
	if status != flow[0] {
		rinfo.GinFail(c, rinfo.CodeState, fmt.Sprintf("项目当前 %s，%s 仅允许 %s → %s", status, req.Action, flow[0], flow[1])); return
	}
...
func (s *Service) ImportSamples(c *gin.Context) {
	...
	if req.Phones != nil && len(req.Phones) != len(req.Names) {
		rinfo.GinFail(c, rinfo.CodeParam, "phones 与 names 长度不一致"); return
	}
	var first int64
