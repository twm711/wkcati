...
> **◐ M3/M4 呼入部分提前落地（2026-09-18，nk3c-go/internal/media/）**：diago **v0.32.2** + sipgo v1.6（需 **Go ≥1.23 工具链**，go.mod 已=1.23.0）；opus 为可选 build tag `with_opus_c`（默认纯 Go 回退，无 CGO）。单进程两域兑现 M0–M4 拆分原则：`cmd/nk3c-api --sip-addr 0.0.0.0:5060` 同进程起 API+话务域。接口隔离：`media.IvrDriver`（StartSIP/KeySIP/HangupSIP）驱动抽取出的 **ivr 核心引擎**（internal/ivr/core.go）——网页模拟按键与真实话机 DTMF 同一走线；提示音运行时合成（8kHz/16bit WAV 多频音，无音频资产依赖）；DTMF 走 RTP telephone-event。**SIP E2E 全绿**（internal/media/sip_e2e_test.go：diago 双端同进程回环：INVITE/200/ACK→提示音→RTP DTMF「0」→TRANSFER:MANUAL→工单 PENDING）。**✅ M2 剩余大部落地（同日第二增量）**：①外呼腿 `OutboundCaller`（diago Invite → 自动问卷：提示音+RTP DTMF → `agent.answerCore/resultCore` 闭环，`POST /api/agent/calls/:callId/dial`）；②**录音落盘**（`AudioStereoRecordingWav` 直通 tap 上叠加 RTP 事件检测——单消费者架构，`recordings/*.wav` + `cti_call_record.record_file`/`ivr_call_log.record_file` 落库 + `GET /api/recording/:callId` 回放）；③**gophone 真机演练 6/6 PASS**（`autotest/sip_drill.sh`：真实呼入→DTMF 0→工单→录音→回放）。核心坑与解法沉淀：RTP 事件依赖 marker 包，**录音泵与 DTMF 泵分读同一管道会互吞 marker**——必须单消费者串链（audio_tap.go）。**✅ M2 收官（同日第三增量）**：①**B2BUA 坐席桥接**（`Bridge()`：客户腿+坐席腿 `InviteBridge` 双腿自动 proxyMedia；客户接通即 `MarkBridgeConnect`（connect_time+样本 INCALL）；任一端挂断桥散、话务保持 OPEN 由坐席经 HTTP 手工提交结果码——真人外呼标准动线，`POST /api/agent/calls/:callId/bridge`，通话中可实时录答案）；②**flow 热更 SIP 验证**（`PUT /api/ivr/flow` 即时生效，真实 SIP 呼入按新流程 w2→m2→b2 走线落库）。SIP E2E 增至 **×5 全绿**（呼入转人工/外呼自动调研/呼入录音/B2BUA 桥接全动线/flow 热更）。遗留至 M5：桥接模式录音（需在 proxyMedia 链上做 tap 拦截）与双音轨。**✅ M3 第一增量（2026-09-20）**：**导出中心 + 实时推送**——①`GET /api/export/:pid/:format`（csv/xlsx/sav）：CSV 带 BOM、XLSX 双工作表、**SAV 系统文件**（数值题=数值变量 F8.2，其余宽 32 字符串；record7 subtype13 长变量名 + subtype20 UTF-8=650 声明；SPSA 头 116B/rec7 头 16B 两处布局坑均已由逐记录解析测试钉死）；②`GET /api/ws/monitor?token=`：监控墙 2s 周期推送（帧 wall），鉴权双通道（?token= 优先、Bearer 回退）；③web：Monitor 页 WS 优先+失败降级轮询（连接状态标签）、项目详情抽屉导出三按钮（window.open ?token=）。E2E ×4 全绿（CSV 解析/XLSX 双表/SAV 逐记录/WS 帧+401），全量回归 + race 绿。**✅ M3 第二增量（同日）**：**质检事件流 + 强签**——①`GET /api/qc/ws?token=`（仅 groupAdmin，升级前 403）：EventHub 事件驱动广播（DIAL/ANSWER/RESULT/FORCE_LOGOUT 帧，无订阅者零开销），事件同步落 `cti_monitor_event`；②`POST /api/qc/force-checkout`：注销目标全部会话（auth.LogoutAll）+ 释放 ASSIGNED/INCALL 样本回池 + 广播；③迁移器泛化多版本顺序应用（002_qc.sql）；④web 监控墙坐席卡强签按钮；质检事件流实时表格（督导专属，断线 5 秒自动重连，保留最近 50 条）。 **✅ M3 第七增量（同日）**：新增 `sys_op_log` 审计迁移和统一中间件，记录写操作及导出/录音访问（不落请求体 PII）。
**✅ M3 第四增量（同日）**：web 导出中心（列清单/列裁剪/本地历史）与 MySQL 8.0 方言演练（InnoDB/utf8mb4、答题 upsert 双方言）。
**✅ M3 第三增量（同日）**：CTI 扩展列迁移 `003_cti_extensions.sql`（录音索引、答题质检时间；Goose 兼容 Up/Down；旧 SQLite 库重复列兼容）。**工程红线新增**：publishQC 严禁在 store.Tx 闭包内调用（单连接池 MaxOpenConns(1) 下 INSERT 等待未提交事务的连接=死锁），必须在事务提交后发布——Dispatch 初版即踩此坑并由测试超时钉出。E2E ×2 全绿（QC 三帧+落库+403；强签四断言），对应 P0 test_17/18，全量回归+race 绿。
...
| 导出中心三格式 | `TestExportCSV`、`TestExportXLSX`、`TestExportSAV`（app 模块） |
| 监控墙 WS 推送 | `TestMonitorWebSocket`（app 模块） |
| test_17/18 质检推送/强签 | `TestQC_WsPush`、`TestQC_ForceCheckout`（app 模块，✅ 已落地） |
